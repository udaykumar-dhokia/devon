...
import { GitBranchPlus, Globe, Loader2, Save, Server, Settings as SettingsIcon } from "lucide-react"
...
export function SettingsModal({ initialConfig, onUpdate }: SettingsModalProps) {
  ...
  const fetchModels = async () => {
    ...
    setError(null)
    try {
      const result = await window.electronAPI.fetchModels(formData.llm_base_url)
      if (result.success) {
        ...
        <div className="grid gap-4 py-4">
          <div className="space-y-4 border-b pb-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="s-username" className="text-right text-xs">Username</Label>
              <Input
                id="s-username"
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="s-token" className="text-right text-xs">Token</Label>
              <Input
                id="s-token"
                type="password"
                className="col-span-3 h-8"
          ...
          <div className="space-y-4">
            <h4 className="font-medium flex items-center gap-2 text-sm">
              <Server className="w-4 h-4" /> Agent Engine
            </h4>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3 flex gap-2">
                ...
                />
                <Button variant="secondary" size="icon" className="h-8 w-8" onClick={fetchModels} disabled={fetchingModels}>
                  {fetchingModels ? <Loader2 className="w-3 h-3 animate-spin" /> : <Globe className="w-3 h-3" />}
                </Button>
              </div>
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="col-span-3">
                <Select 
                  ...
                  onValueChange={(val) => setFormData({ ...formData, llm_model_name: val })}
                >
                  ...
                  <SelectContent>
                    {models.length > 0 ? (
                      ...
                      <SelectItem value={formData.llm_model_name}>{formData.llm_model_name}</SelectItem>
                    )}
                  </SelectContent>
                </Select>
              </div>
