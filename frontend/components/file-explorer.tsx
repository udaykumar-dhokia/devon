...
function TreeItem({ item, depth, onFileDoubleClick }: TreeItemProps) {
  ...
  return (
    <div className="select-none">
      <div
        className={cn(
          "flex items-center py-1 px-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer rounded-none group",
          "text-sm transition-colors duration-200",
        )}
        style={{ paddingLeft: `${depth * 12 + 8}px` }}
        onClick={() => isDirectory && setIsOpen(!isOpen)}
        onDoubleClick={() =>
          !isDirectory && item.path && onFileDoubleClick(item.path)
        }
      >
